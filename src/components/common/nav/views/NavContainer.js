...
class NavContainer extends Component {
  ...
  navLiClick(event, index, item) {
    ...
    // 判断路由跳转的路径
    if (item.categoryId === 623) {
      this.props.history.push('/')
    } else {
      this.props.history.push(`/category?categoryId=${item.categoryId}`)
    }
            ...
            <ul>
              {
                this.state.categoryList.map((item, index) => (
                  <CategoryListLi
                    ...
                    onTouchStart={(event) => this.navLiClick(event, index, item)}
